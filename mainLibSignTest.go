...
func AsymmetricEncryptDoubleSign_test() {
	...
	//sum := md5.Sum([]byte(s1))
	//fmt.Println("sum:", sum)
	//dstr := AsymmetricDecryptDoubleSign(C.CString(p2), C.CString(s1), estr, C.CString("rsa"), C.CString("rsa"))
	//fmt.Println(C.GoString(dstr))
	//decodeString, err = base64.StdEncoding.DecodeString(C.GoString(dstr))
	//if err != nil {
	//	panic(err)
	//}
	//fmt.Println("end")
